...
function DemoVideo({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Phase 1: Search animation (0-5s)
      if (elapsed < 5) {
        const searchProgress = elapsed / 5;
      ...
      // Phase 3: Worker walks to product (8-18s)
      else if (elapsed < 18) {
        const walkProgress = (elapsed - 8) / 10;
        const path = [
          { x: 50, y: 280 },
          { x: 200, y: 280 },
          { x: 200, y: 150 },
      ...
      // Phase 4: Switch to 3D view (18-24s)
      else if (elapsed < 24) {
        setViewMode('3d');
        setRotation((elapsed - 18) * 30);
      }
      // Phase 5: Show stats (24-30s)
      else {
        setViewMode('stats');
      }
          ...
          {viewMode === 'stats' && (
            <div className="video-stats-view">
              ...
              <div className="cta-area">
                <button className="video-cta" onClick={handleClose}>🚀 Try WareTrack Now</button>
              </div>
            </div>
          )}

          <div className="phase-indicator">
            {currentTime < 5 && '🔍 Searching for product...'}
            {currentTime >= 5 && currentTime < 8 && '📍 Product found! Calculating route...'}
            {currentTime >= 8 && currentTime < 18 && '🚶 Following navigation to product...'}
            {currentTime >= 18 && currentTime < 24 && '🎮 Exploring 3D view...'}
            {currentTime >= 24 && '📈 See your potential savings!'}
          </div>
        </div>
